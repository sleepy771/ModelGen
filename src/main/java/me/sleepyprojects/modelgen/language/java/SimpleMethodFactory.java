...
public class SimpleMethodFactory implements MethodFactory<JavaMethodType> {
    ...
    // @Inject
    public SimpleMethodFactory(MetaFactory<JavaModifierType> modifierFactory, MetaFactory<JavaAnnotationType> annotationFactory, VariableFactory<JavaArgumentType> argumentFactory) {
    ...
    public JavaMethodType create(MethodDefinition definition) {
        ...
        definition.getMetas().stream().filter(modifierFactory::is).forEach(meta -> jMethod.addModifier(modifierFactory.get(meta)));
        definition.getMetas().stream().filter(annotationFactory::is).forEach(meta -> jMethod.addAnnotation((AnnotationType<JavaMarker>) annotationFactory.get(meta)));
        definition.getArgs().stream().forEach(arg -> jMethod.addArgument(argumentFactory.create(arg)));
